 ...
 * Template Browser & Custom Template Creation Modals for TikZ Block.
 */

import {
  GetAllTikzTemplates,
  SaveCustomTikzTemplate,
  DeleteCustomTikzTemplate
} from './Tikz_Templates.js';
import { escapeHtml } from '../../02_Utils.js';

/**
 * Returns HTML markup for the TikZ Template Browser & Save Template Modal.
 */
export function GetTikzTemplatesModalHTML() {
  return `
    <!-- Template Browser Window (Modeled after 02_Group_Filter.js) -->
    <div class="template-browser-window hidden absolute top-9 left-0 z-50 w-full max-w-[620px] bg-[var(--surface,#181b27)] border border-[var(--border,#2a2e40)] rounded-xl shadow-2xl p-3.5 backdrop-blur-xl box-border">
      ...
      <div class="flex items-center justify-between gap-2 pb-2.5 mb-2 border-b border-[var(--border)]">
        <div class="relative flex-1 min-w-0">
          ...
          <input type="text" class="template-search-input w-full h-7 pl-8 pr-2 rounded-lg border border-[var(--border)] bg-[var(--card,#1c1f2e)] text-xs text-[var(--text)] outline-none focus:border-[var(--accent,#8b6dff)] transition-colors" placeholder="Search templates..." />
        </div>
        ...
        <div class="flex items-center gap-1 bg-[var(--card,#1c1f2e)] border border-[var(--border)] px-1.5 py-0.5 rounded-lg text-xs flex-shrink-0 select-none">
          <button type="button" class="btn-col-dec px-1 font-bold text-[var(--text-secondary)] hover:text-white cursor-pointer">&minus;</button>
          <span class="col-count-val font-bold text-[var(--accent,#8b6dff)] min-w-[14px] text-center">2</span>
          <button type="button" class="btn-col-inc px-1 font-bold text-[var(--text-secondary)] hover:text-white cursor-pointer">&plus;</button>
        </div>
    ...
    <div class="save-template-modal hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div class="bg-[var(--surface,#181b27)] border border-[var(--border,#2a2e40)] rounded-xl max-w-md w-full p-4 shadow-2xl flex flex-col gap-3">
        <div class="flex items-center justify-between border-b border-[var(--border)] pb-2">
          <h4 class="text-xs font-bold text-[var(--text)]">Save TikZ Template</h4>
        ...
        <div class="flex flex-col gap-1">
          ...
          <input type="text" class="input-template-name w-full h-8 px-2.5 rounded-lg border border-[var(--border)] bg-[var(--card,#1c1f2e)] text-xs text-[var(--text)] outline-none focus:border-[var(--accent,#8b6dff)]" placeholder="e.g. Optical Cavity with 2 Mirrors" />
        </div>

        <div class="flex flex-col gap-1">
          <label class="text-[11px] font-medium text-[var(--text-secondary)]">Category / Group</label>
          <input type="text" class="input-template-category w-full h-8 px-2.5 rounded-lg border border-[var(--border)] bg-[var(--card,#1c1f2e)] text-xs text-[var(--text)] outline-none focus:border-[var(--accent,#8b6dff)]" placeholder="Physics, Geometry, Custom..." list="tikz-category-datalist" value="Custom" />
          <datalist id="tikz-category-datalist">
            <option value="Physics & Optics"></option>
            <option value="Geometry & Math"></option>
            <option value="Flowcharts & Diagrams"></option>
 ...
 */
export function InitTikzTemplatesLogic(parentEl, { onInsert = null, getCurrentCode = () => '' } = {}) {
  if (!parentEl) return { cleanup: () => {} };
  ...
  const renderBrowser = () => {
    const allTemplates = GetAllTikzTemplates();
      ...
      templatesGrid.querySelectorAll('.template-tile').forEach(tile => {
        ...
        tile.querySelector('.btn-delete-tpl')?.addEventListener('click', (e) => {
          ...
          if (confirm(`Delete template "${targetTpl.name}"?`)) {
            DeleteCustomTikzTemplate(targetTpl.id);
            renderBrowser();
          }
        });
      });
  ...
  btnTemplates?.addEventListener('click', (e) => {
    ...
    if (!browserWindow.classList.contains('hidden')) {
      renderBrowser();
      searchInput?.focus();
    }
  });
  ...
  btnSaveTemplate?.addEventListener('click', (e) => {
    ...
    if (!currentCode) {
      alert('Please enter some TikZ code first before saving as a template.');
      return;
    }
  ...
  btnConfirmSave?.addEventListener('click', (e) => {
    e.stopPropagation();
    const name = inputTemplateName?.value.trim() || 'Custom TikZ Diagram';
    const cat = inputTemplateCategory?.value.trim() || 'Custom';
    const code = getCurrentCode().trim();

    SaveCustomTikzTemplate({ name, category: cat, code });
    closeSaveModal();
  ...
  const onDocClick = (e) => {
    if (browserWindow && !browserWindow.contains(e.target) && btnTemplates && !btnTemplates.contains(e.target)) {
      browserWindow.classList.add('hidden');
    }
  };
